...
RESULTS_DIR = '/n/regal/pfister_lab/PERCEPTION/RESULTS_FROM_SCRATCH/' + EXPERIMENT + '/'
OUTPUT_DIR = '/n/regal/pfister_lab/PERCEPTION/CROSSNETWORK_FROM_SCRATCH/' + EXPERIMENT +'/'
...
for data in range(DATASETS):
  ...
  for network in range(DATASETS):
    ...
    # make 3d
    reshaped_data = np.stack((X[data],)*3, -1)
...
def plot_cross_network(cm, classes, title, cmap=plt.cm.Blues, c_name='', e_name='', c_index=-1):
    ...
    plt.savefig('../PAPER/SUPPLEMENTAL/gfx/NEW_cn_'+str(e_name)+'_'+str(c_name)+'_'+str(c_index)+'.pdf', bbox_inches='tight', pad_inches=0)
